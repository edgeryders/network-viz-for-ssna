...
def main(graph):
    ...
    myAnnos = api.fetch_annos('ethno-poprebel')
    codes = api.fetch_codes_from_annos(myAnnos) # remember to delete this
    for code in codes:
        print(code['id'])
        for n in graph['code_id'].getNodesEqualTo(str(code['id'])):
